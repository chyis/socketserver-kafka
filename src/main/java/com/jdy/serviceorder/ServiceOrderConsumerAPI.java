 ...
 */
public class ServiceOrderConsumerAPI extends Thread{
    // 最大重试次数
    ...
    //static LogOut logger = new LogOut(LogManager.getLogger(ServiceOrderConsumerAPI.class.getName()));
    ...
    public void run(long maxReads,
                    ...
                    List<KafkaBrokerInfo> seedBrokers, ChannelHandlerContext responseHandle) throws Exception {
        ...
        try {
            ...
            //logger.log("info","The first read offset is:" + readOffSet);
            ...
            while (ever || maxReads > 0) {
                ...
                if (response.hasError()) {
                    ...
                    //logger.log("error", "Error fetching data from the Broker:" + leaderBrokerHost + " Reason:" + code);
                ...
                for (MessageAndOffset messageAndOffset : response.messageSet(topic, partitionID)) {
                    ...
                    if (currentOffset < readOffSet) {
                    	//logger.log("error","Found and old offset:" + currentOffset + " Expection:" + readOffSet);
                        continue;
                    }
            		...
            		String jsonData = gson.toJson(result);// 由Gson生成json字符串
            		
                    String resultData = "[top]"+jsonData+"[end]";
                    //逻辑结束了，最后返回然后更新偏移量
                ...
                if (numRead == 0) {
                    ...
                    //    String resultData = "[top]{\"result\":\"ok\",\"resulttype\":\"heartbeat\",\"actiondata\":\"\"}[end]";
					//	byte[] datas = resultData.getBytes();
					//	ByteBuf buf_out = Unpooled.copiedBuffer(datas);
					//	
					//	responseHandle.writeAndFlush(buf_out);
